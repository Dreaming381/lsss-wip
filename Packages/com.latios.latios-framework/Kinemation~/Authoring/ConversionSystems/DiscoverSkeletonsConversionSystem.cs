...
namespace Latios.Kinemation.Authoring.Systems
{
    ...
    [DisableAutoCreation]
    public partial class DiscoverSkeletonsConversionSystem : GameObjectConversionSystem
    {
        protected override void OnUpdate()
        {
            var skinnedMeshToAuthoringEntityDictionary = new Dictionary<SkinnedMeshRenderer, Entity>();
            Entities.ForEach((Entity entity, SkinnedMeshRenderer renderer) => skinnedMeshToAuthoringEntityDictionary.Add(renderer, entity)).WithoutBurst().Run();
            ...
            Entities.ForEach((Entity entity, Animator animator) =>
            {
                ...
                ecb.AddComponent(entity, context);
                DiscoverSkinnedMeshes(context, skinnedMeshToAuthoringEntityDictionary, ref ecb);
            }).WithoutBurst().Run();
        ...
        void DiscoverSkinnedMeshes(SkeletonConversionContext skeletonContext, Dictionary<SkinnedMeshRenderer, Entity> smrToAuthoringEntityDictionary, ref EntityCommandBuffer ecb)
        {
            m_smrCache.Clear();
            Transform root;
            ...
            foreach (var smr in m_smrCache)
            {
                ...
                if (authoring != null)
                {
                    if (authoring.bindingMode == BindingMode.Import)
                    {
                        ...
                        ecb.AddComponent(GetPrimaryEntity(authoring), meshContext);
                        continue;
                    }
...

                {
                    ...
                    ecb.AddComponent(smrEntity, meshContext);
                }
            }
        }
    }
