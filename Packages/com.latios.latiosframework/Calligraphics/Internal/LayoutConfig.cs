...
namespace Latios.Calligraphics
{
    internal struct LayoutConfig
    {
        ...
        public half4                     m_htmlColor;
        public FixedStack512Bytes<half4> m_htmlColorStack;
        public half4                     m_underlineColor;
        public FixedStack512Bytes<half4> m_underlineColorStack;
        public half4                     m_strikethroughColor;
        public FixedStack512Bytes<half4> m_strikethroughColorStack;
        ...
        public LayoutConfig(in TextBaseConfiguration textBaseConfiguration)
        {
            ...
            m_underlineColor     = Color.white.ToHalf4();
            m_strikethroughColor = Color.white.ToHalf4();
        ...
        public void Reset(in TextBaseConfiguration textBaseConfiguration)
        {
            ...
            m_htmlColor          = textBaseConfiguration.color;
            m_underlineColor     = (half)1f;
            m_strikethroughColor = (half)1f;
            useGradient          = false;
            m_gradient           = default;
            m_gradientStack      = default;
        ...
        internal void Update(ref XMLTag tag, in TextBaseConfiguration textBaseConfiguration, ReadOnlySpan<TextColorGradient> textColorGradients)
        {
            switch (tag.tagType)
            {
                ...
                case TagType.Alpha:
                    if (tag.value.valueLength != 3)
                        return;
                    m_htmlColor.w = (half)tag.value.ColorValue.a;
                    return;
                case TagType.Align:
                    if (!tag.isClosing)
                    {
                ...
                case TagType.Color:
                    if (!tag.isClosing)
                    {
                        ...
                        if (tag.value.type == TagValueType.ColorValue)
                            m_htmlColor = tag.value.ColorValue.ToHalf4();
                        else if (tag.value.type == TagValueType.StringValue)
                        {
                            switch (tag.value.stringValue)
                            {
                                case StringValue.red:  // <color=red>
                                    m_htmlColor = Color.red.ToHalf4();
                                    break;
                                case StringValue.lightblue:  // <color=lightblue>
                                    m_htmlColor = new Color32(173, 216, 230, 255).ToHalf4();
                                    break;
                                case StringValue.blue:  // <color=blue>
                                    m_htmlColor = Color.blue.ToHalf4();
                                    break;
                                case StringValue.grey:  // <color=grey>
                                    m_htmlColor = new Color32(128, 128, 128, 255).ToHalf4();
                                    break;
                                case StringValue.black:  // <color=black>
                                    m_htmlColor = Color.black.ToHalf4();
                                    break;
                                case StringValue.green:  // <color=green>
                                    m_htmlColor = Color.green.ToHalf4();
                                    break;
                                case StringValue.white:  // <color=white>
                                    m_htmlColor = Color.white.ToHalf4();
                                    break;
                                case StringValue.orange:  // <color=orange>
                                    m_htmlColor = new Color32(255, 128, 0, 255).ToHalf4();
                                    break;
                                case StringValue.purple:  // <color=purple>
                                    m_htmlColor = new Color32(160, 32, 240, 255).ToHalf4();
                                    break;
                                case StringValue.yellow:  // <color=yellow>
                                    m_htmlColor = Color.yellow.ToHalf4();
                                    break;
                            }
                        }
                        m_htmlColorStack.Add(m_htmlColor);
                    ...
                    if (!tag.isClosing)
                    {
                        switch (tag.value.unit)
                        {
                            ...
                            case TagUnitType.FontUnits:
                                ...
                                break;
                            case TagUnitType.Percentage:
                                //fontScale = (richtextAdjustments.m_currentFontSize / m_currentFontAsset.faceInfo.fontSize * m_currentFontAsset.faceInfo.scale * (richtextAdjustments.m_isOrthographic ? 1 : 0.1f));
